...
export default async function jobConfigRoutes(fastify, options) {
  ...
  // Add this route to jobConfigRoutes.js, right after the toggle route

  // Get job config status
  fastify.get('/job-config/status', { onRequest: [fastify.authenticate] }, async (request, reply) => {
    try {
      const userId = request.user.id;
      ...
      fastify.log.error({ err: error }, 'Error getting job config status');
      return reply.code(500).send({
        error: 'Server Error',
        message: error.message
      });
